...
Step 3.10: UI 구현 (UI Implementation)
...
- Step 3.9 (Core Features) 완료 후 실행
...
> **⚠️ UI 구현 전 반드시 `project-setup.md`의 스타일 매핑 테이블을 확인하세요.**
> **매핑 테이블에 없는 클래스는 절대 사용하면 안 됩니다!**

#### 클래스 사용 전 검증 절차

```
모든 스타일 클래스 사용 전:
1. project-setup.md의 스타일 매핑 테이블 확인
2. 시맨틱 토큰에 해당하는 구현 클래스 찾기
3. 해당 클래스만 사용

예시:
- Primary 버튼 배경 필요 → 매핑 테이블에서 button-primary-bg 확인
  → bg-gradient-primary 사용 (✅)
  → btn-primary 사용 (❌ 매핑 테이블에 없음)
```

#### 디자인 시스템 준수 규칙

| 하려는 것 | 매핑 테이블에서 확인할 토큰 | 사용할 클래스 |
|----------|---------------------------|--------------|
| Primary 버튼 배경 | button-primary-bg | {매핑된 클래스} |
| Primary 버튼 텍스트 | button-primary-text | {매핑된 클래스} |
| H1 스타일 | text-heading-1 | {매핑된 클래스} |
| 주요 색상 | color-primary | {매핑된 클래스} |
| 기본 간격 | spacing-4 | {매핑된 클래스} |
...
> **⚠️ 이 규칙을 어기면 디자인 시스템이 깨집니다!**

1. **매핑 테이블에 없는 클래스 사용 금지**
   - 모든 스타일 클래스는 `project-setup.md`의 스타일 매핑 테이블에 정의되어 있어야 함
   - 예: `btn-primary`, `card-shadow` 등 임의의 클래스 사용 금지

2. **시맨틱 토큰 우회 금지**
   - 디자인 값을 직접 하드코딩하지 않음
   - 예: `bg-[#3B82F6]` 대신 매핑된 `bg-primary` 사용

3. **정의되지 않은 색상/간격/크기 사용 금지**
   - 디자인 시스템에 정의된 값만 사용
   - 예: `p-7` (디자인 시스템에 없음) 대신 `p-8` (디자인 시스템에 있음) 사용

4. **클래스 사용 전 검증 필수**
   ```
   클래스 사용 전 체크:
   □ 이 클래스가 매핑 테이블에 있는가?
   □ 없다면, 어떤 시맨틱 토큰에 해당하는가?
   □ 해당 토큰의 매핑된 클래스를 사용했는가?
   ```

5. **새로운 스타일이 필요한 경우**
   - 임의로 클래스를 만들지 않음
   - Bridge로 돌아가서 시맨틱 토큰 정의
   - Project Setup에서 매핑 추가
   - 그 후 UI 구현에서 사용

### UI 콘텐츠 가이드라인

- **이모티콘 사용 최소화**: UI에서 이모티콘 사용을 최소화합니다. 이모티콘이 많으면 페이지가 어색해지고 전문성이 떨어져 보입니다. 꼭 필요한 경우에만 제한적으로 사용하세요.
...
- 기능 연동 완료
- 반응형 동작 확인
- **디자인 시스템 준수 확인** - 모든 스타일 클래스가 매핑 테이블에 정의됨
- **미정의 클래스 없음 확인** - 임의의 스타일 클래스 사용하지 않음
- `ui-impl.md` 파일이 생성됨
