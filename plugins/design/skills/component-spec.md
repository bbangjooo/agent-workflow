...
Step 2.5: 공통 컴포넌트 명세 정의

## 설명

screen-structure의 화면들에 필요한 UI 컴포넌트를 정의하는 스킬입니다. 디자인 시스템을 바탕으로 **공통 컴포넌트를 먼저 정의하고, 이것만으로 모든 화면을 조립**할 수 있게 명세합니다.

**플랫폼에 따라 별도의 산출물을 생성합니다.** Web과 Mobile은 서로 다른 컴포넌트 라이브러리와 패턴을 사용합니다.

> 핵심 원칙: "AI에게 큰 결정을 맡기지 마라. 프레임워크 선택, 라이브러리 설치, 초기 설정은 시니어(인간)가 직접 한다."
> 공통 컴포넌트를 정확하게 만들면 페이지 구현 프롬프트가 매우 짧아진다 — 체계적 접근의 진정한 위력.

### 올바른 순서 (필수)
```
1. 테마 설정 (globals.css) — 이미 완료
2. 공통 컴포넌트 제작 — 이 Step
3. 페이지 구현 — 다음 Stage에서
```

> 테마를 CSS로 설정했지만 공통 컴포넌트를 제작하지 않으면 → 중복 코드 폭발 + 일관성 붕괴.
> **공통 컴포넌트 없이 페이지 구현으로 넘어가는 것은 금지.**

## 트리거

- Step 2.4 (Design Tokens) 완료 후 자동 실행
- `outputs/stage-2/ui/04-tokens.md` 파일이 존재할 때
...
플랫폼 선택에 따라 다른 파일이 생성됩니다 (Shortify 트리):

- **Web**: `outputs/stage-2/ui/05-components-web.md`
- **Mobile**: `outputs/stage-2/ui/05-components-mobile.md`
- **Both**: 두 파일 모두 생성

각 산출물 상단에 메타 헤더 + 하단에 변경 이력 표를 필수 포함한다.

---

### Web 컴포넌트 명세 템플릿

`outputs/stage-2/ui/05-components-web.md`
...
stage: 2
step: "2.5 — Component Spec (Web)"
platform: web
ui_library: "{shadcn/ui | Material UI | Chakra UI | etc.}"
---
...
`outputs/stage-2/ui/05-components-mobile.md`
...
last_updated: {YYYY-MM-DD}
stage: 2
step: "2.5 — Component Spec (Mobile)"
platform: mobile
ui_library: "{React Native Paper | NativeBase | Flutter Material}"
---
...
- 컴포넌트 사용처(화면) 매핑됨
- **Web**: `outputs/stage-2/ui/05-components-web.md` 파일 생성
- **Mobile**: `outputs/stage-2/ui/05-components-mobile.md` 파일 생성
- 메타 헤더 + 변경 이력 표 포함
- **Both**: 두 파일 모두 생성
